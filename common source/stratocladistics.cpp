...
if (keep==1)	{
	...
	fprintf(output,"\t� Character Compabibility (SC)");					/* added 2011-10-14	*/
	fprintf(output,"\t� Character Compabibility (SIN)");					/* added 2011-10-14	*/
	fprintf(output,"\t� Compabibility Diff (SC-SIN)\n");				/* added 2011-11-01	*/
...
if (keep==1)	{
	...
	fprintf(output,"\t� Character Compabibility (SC)");					/* added 2011-10-14	*/
	fprintf(output,"\t� Character Compabibility (SIN)");				/* added 2011-10-14	*/
	fprintf(output,"\t� Compabibility Diff (SC-SIN)");					/* added 2011-11-01	*/
	fprintf(output,"\t� H00 CoG");										/* added 2013-04-16	*/
	fprintf(output,"\t� D00 CoG");										/* added 2013-04-16	*/
	fprintf(output,"\t� Clade CoG");									/* added 2013-04-16	*/
...
/** datecladerealape - Finds age of clades based on the oldest member in the clade
/*		NOTE: if you have branch lengths, use "datecladesExtra - this allows for ancestors
/* Requires:
/*	ranges - First Appearances and last appearances in continuous numbers (e.g., millions of years ago)
/*	tree - a vector stating the ancestor of each species and node;
/*	nodes - number of nodes (clades)
/*	notu - number of taxa;
/*	sign: -1 means that -400 comes before -395
		   1 means that 1 comes before 2
/*	mode: 0; budding: ancestor can co-exist with descendant
		  1: bifucating: anagenetic loss of ancestor
****************************************************************************/
void datecladerealape(long *ape, double **ranges, double **divergences, int nodes, int notu, unsigned long *ancestral, unsigned long *minbr)
{
int	c, d;

for (c=notu; c<(nodes+notu); ++c)	divergences[c][0]=divergences[c][1]=RAND_MAX;
/*if (sign==(-1))		for (c=notu; c<(nodes+notu); ++c)	divergences[c][0]=divergences[c][1]=RAND_MAX;
else if (sign==1)	for (c=notu; c<(nodes+notu); ++c)	divergences[c][0]=divergences[c][1]=-1*RAND_MAX;	*/

/* downward pass #1	(d=daughter)	*/
for (d=(notu-1); d>=0; --d)	{
	divergences[d][0]=ranges[d][0];
	divergences[d][1]=ranges[d][0];
	if (divergences[d][0]<divergences[c=ape[d]][0])	{
		divergences[c][1]=divergences[c][0]=divergences[d][0];	/* do this for budding and bifurcating to allow P[species | time, lambda]	*/
		}
	}	/* end node dating with species	*/

/* downward pass #2	*/
for (d=(notu+nodes)-1; d>notu; --d)	{
	if (divergences[d][0]<divergences[c=ape[d]][0])	{
		divergences[c][1]=divergences[ape[d]][0]=divergences[d][0];
/*			divergences[c][1]=divergences[d][0];	*/
		}
/*	if (mode==BUD && divergences[d][0]>divergences[c=ape[d]][1])
		divergences[c][1]=divergences[d][0];	/* push last appearance of ancestor to first appearances of youngest descendant	*/
	}	/* end node dating with nodes	*/

/* upward pass: pull descendants down to base of node	*/
for (c=notu; c<(notu+nodes); ++c)	{
	for (d=0; d<(notu+nodes); ++d)	{
		if (ape[d]==c && divergences[d][0]>divergences[c][1])	divergences[d][0]=divergences[c][1];
		}	/* end examination of other taxa for descendants	*/
	}	/* end daughter taxon re-dating	*/
}
...
{
int	a, d;
int	f1=0, desc[100], foote=-1;
double x, ble=RAND_MAX;
double	blu=-1*RAND_MAX;
double	div[100];

/*blu=-1*(ble=RAND_MAX);		/* early and upper bounds on divergence	*/
for (d=0; d<nodes+notu; ++d)	{
	if (ape[d]==HTU)	{
		if (d<notu && ancestral[d]>0)	foote=d;	/* ancestor present: only need to redate that species	*/
...
dshellsort_dec_command(div,f1);
	
a=0;
while (div[a]>ble && a<f1)	++a;	/* div[a] should be the youngest acceptable divergence time	*/

if (div[a]!=divergences[HTU][1])	{
	x=fabs(divergences[HTU][0]-divergences[HTU][1]);
	divergences[HTU][1]=div[a];
	if (divergences[HTU][0]>=divergences[HTU][1])	divergences[HTU][0]=divergences[HTU][1]-(x/2);
	}	/* shift divergence date if necessary	*/
	/* EUREKA!!!! 2014-09-17: the budding vs. bifurcating dilemma is solved by keeping branching time at ble, I think!!!!!	*/
if (foote==-1)	{
	for (a=0; a<f1; ++a)	{
		d=desc[a];		/* daughter taxon	*/
		if (divergences[d][0]>divergences[HTU][1])	divergences[d][0]=divergences[HTU][1];
		}	/* redate younger daughter taxon if need be	*/
	}
	else	divergences[foote][0]=divergences[HTU][1];
	if (ape[HTU]!=TREE_END)	datecladerealape_pulley(ape[HTU], ape, divergences, nodes, notu, ancestral);	/* recursion is our friend	*/
}	/* do another version with stratigraphic likelihood!	*/
...
for (d=0; d<f1; ++d)	{
	sp=desc[d];
	desc[d]=sp;;	/* make list of descendants	*/
	div[d]=divergences[sp][0];
	if (ancestral[sp]>0 && sp<notu)	foote=sp;
	if (divergences[sp][1]<ble)	ble=divergences[sp][1];	/* oldest divergence time among descendants		*/
	if (divergences[sp][0]>blu)	blu=divergences[sp][0];	/* youngest divergence time among descendants	*/
	}
...
/** datecladerealape_addexp - Finds age of nodes based on the oldest member in the clade and add exponential branch length
/*		NOTE: if you have branch lengths, use "datecladesExtra - this allows for ancestors
/* Requires:
/*	ranges - First Appearances and last appearances in continuous numbers (e.g., millions of years ago)
/*	ape - a vector stating the ancestor of each species and node;
/*	nodes - number of nodes (clades)
/*	notu - number of taxa;
/*	sign: -1 means that -400 comes before -395
		   1 means that 1 comes before 2
****************************************************************************/
void datecladerealape_addexp_old(long *ape, double **ranges, double **divergences, int nodes, int notu, double lambda)
{
int	c, d;
double x, y, added[1000];

//added=dvector(nodes+notu);
//for (d=0; d<notu+nodes; ++d)	added[d]=exponentialrandom(lambda);
...
for (d=(notu-1); d>=0; --d)	{
	...
	if (divergences[d][0]<divergences[c=ape[d]][0])	{
		...
		y=exponentialrandom(lambda);
		divergences[c][0]=divergences[d][0]-(added[c]=dmax(x,y));		/* add some exponentially distributed random length to the branch	*/
		divergences[c][1]=divergences[d][0]-dmin(x,y);					/* move last appearances of ancestor to first appearance of descendant, plus some distance		*/
		}	/* end case where descendant is older the prior oldest case	*/
	}	/* end node dating with species	*/

/* downward pass #2	*/
for (d=(notu+nodes)-1; d>notu; --d)	{
	if (divergences[d][0]<divergences[c=ape[d]][0])	{
		if (added[c]==0)	added[c]=exponentialrandom(lambda);			/* added exponential length	*/
		divergences[c][0]=divergences[d][0]-added[c];
		divergences[c][1]=divergences[d][0];
		}
	}	/* end node dating with nodes	*/

/* upward pass: pull descendants down to base of node OR put top of node to latest descendant	*/
for (c=notu; c<(notu+nodes); ++c)	{
	for (d=0; d<(notu+nodes); ++d)	{
		if (ape[d]==c)	{
			if (divergences[d][0]>divergences[c][1])	divergences[d][0]=divergences[c][1];	/* if any descendants are younger than node, then redate them IF mode==BIF	*/
/*			if (mode==BUD && divergences[d][0]>divergences[c][1])	divergences[c][1]=divergences[d][0];	/* if any descendants are younger than node, then redate end of node IF mode==BUD	*/
			}	/* end examination of descendant	*/
		}	/* end examination of other taxa for descendants	*/
	}	/* end daughter taxon re-dating	*/
...
int	c, d, f;
int desc[10];
double adds[10], fas[10];

for (d=0; d<notu; ++d)	divergences[d][1]=ranges[d][0];	/* set top of divergence time to first appearance	*/
for (c=(notu+nodes)-1; c>=notu; --c)	{
	f=0;
	for (d=0; d<(notu+nodes); ++d)	{
		if (ape[d]==c)	{
			fas[f]=divergences[desc[f]=d][1];
			adds[f]=exponentialrandom(lambda);
			++f;
			}
		}
	dshellsort_inc_command(adds,f);	/* sort smallest increases to largest	*/
	dshellsort_inc_command(fas,f);	/* sort largest increases to smallest	*/
	for (d=0; d<f; ++d)	fas[d]-=adds[d];	/* move fa up a bit	*/
	divergences[c][0]=divergences[c][1]=mindarray(fas,f);
	for (d=0; d<f; ++d)	divergences[desc[d]][0]=divergences[c][1];
	}
}
...
for (d=0; d<(notu+nodes); ++d)	ma_htu+=(divergences[d][1]-divergences[d][0]);
