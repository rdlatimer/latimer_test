...
***************************************************************************************************************************/
double *stratcompatfullplusplus(long **ranges, long **matrix, int *states, unsigned long *charcomps, int ch1, int ch2, int notu, int UNKNOWN, int INAP)
{
int	a, b, c, d, f, p;
...
/*						if (prfl[swing][0]>prfl[end1][0] && prfl[swing][0]<=prfl[end2][0])	{  2013-04-12: WTF?!?!  2nd comparison not needed	*/
						...
						/* if the first two morphotypes appear at the same time, then consider it hierarchical if only the swing pair
								is present when the final pair appears (or it if is the last present); otherwise, tally it as a split	*/
...
/*	ch2 - 2nd character being compared;
/*	notu - number of taxa;
/*	c0 - ∑d (i.e., sum of lineage segments);
/*	UNKNOWN - coding for an unknown character;
/*	INAP - coding for an inapplicable character;
/*
/* Returns:
/*	stratcomp - an array telling you whether the character shows gaps in its  stratigraphic range or not.
		...
		stratcomp[18]: when ancestral pair goes extinct;
****************************************************************************************************************************/
double *stratcompatfullplusplusplus(long **ranges, long **matrix, int *states, unsigned long *charcomps, int ch1, int ch2, int notu, double c0, int onset, int end, int UNKNOWN, int INAP)
{
int	a, b, c, d, f, p, t;
int	maxocc, st1, st2, ttlprs=0, maxst;
//int	cc, sp, maxocc, st1, st2, sc1, sc2, rc, scf1, scf2, scu1, scu2, prs, m1, m2, ttlprs=0, maxst;
...
long prfl[3][2];
double *stratcomp;
double *cg;												/* 2013-04-12: center of gravity	*/
int ancp;												/* 2013-04-12: ancestral pair		*/
//long pairs[3][2];
...
stratcomp=dvector(19);
cleardvector(stratcomp,17,0);
...
CG=dvector(RUNS);				/* tallies for simmary[30]								*/
HAG=dvector(RUNS);				/* tallies for simmary[31]								*/
DAG=dvector(RUNS);				/* tallies for simmary[31]								*/
	...
	for (c1=0; c1<(nchars-1); ++c1)	{
		...
		for (c2=(c1+1); c2<nchars; ++c2)	{
			...
			if (compmat[c1][c2]==1)	{
				...
				sstrcmp=stratcompatfullplusplusplus(ranges, simatrix, nstates, charcomps, c1, c2, notu, sC[0], onset, end, UNKNOWN, INAP);
